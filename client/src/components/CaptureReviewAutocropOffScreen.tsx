...
interface CaptureReviewAutocropOffScreenProps {
  onNavigate: (screen: string) => void;
  onSavePhoto: (photoData: any) => void;
  capturedImage: string | null;
}

export function CaptureReviewAutocropOffScreen({ onNavigate, onSavePhoto, capturedImage }: CaptureReviewAutocropOffScreenProps) {
  const [autoCrop, setAutoCrop] = useState(false);
  const [showGrid, setShowGrid] = useState(true); // Increased contrast for precision
  ...
  return (
    <div className="w-full h-full min-h-screen bg-gradient-to-b from-[#0A0A0A] to-[#1A1A1A] relative overflow-hidden max-w-[393px] mx-auto">
      ...
      <div className="relative w-full h-full flex items-center justify-center px-8 pt-8">
        ...
        >
          {/* Realistic Receipt Content - Same as main screen */}
          ...
          {/* Manual Crop Overlay - Auto-crop OFF */}
          <div className="absolute inset-0 pointer-events-auto">
            {/* Dashed polygon border - user adjustable, wider crop area */}
            <svg className="absolute inset-0 w-full h-full">
              <defs>
                <filter id="glow-manual">
              ...
              <polygon
                points="40,50 260,45 255,375 35,380"
                fill="none"
                stroke="white"
                strokeWidth="2"
            ...
            {/* Corner Handles - Draggable, larger for manual mode */}
            <div className="absolute w-4 h-4 bg-white rounded-full shadow-lg border-2 border-gray-300 cursor-grab hover:cursor-grabbing hover:scale-110 transition-all duration-200" style={{ top: '48px', left: '38px' }}></div>
            <div className="absolute w-4 h-4 bg-white rounded-full shadow-lg border-2 border-gray-300 cursor-grab hover:cursor-grabbing hover:scale-110 transition-all duration-200" style={{ top: '43px', right: '38px' }}></div>
            <div className="absolute w-4 h-4 bg-white rounded-full shadow-lg border-2 border-gray-300 cursor-grab hover:cursor-grabbing hover:scale-110 transition-all duration-200" style={{ bottom: '43px', right: '43px' }}></div>
            <div className="absolute w-4 h-4 bg-white rounded-full shadow-lg border-2 border-gray-300 cursor-grab hover:cursor-grabbing hover:scale-110 transition-all duration-200" style={{ bottom: '38px', left: '33px' }}></div>
          </div>
      ...
      {/* Tool Tray - Same improved design */}

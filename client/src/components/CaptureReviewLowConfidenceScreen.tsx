...
import { RotateCw, Grid3X3, AlertTriangle } from 'lucide-react';
import { Button } from './ui/button';
import { Slider } from './ui/slider';
import { toast } from 'sonner';

interface CaptureReviewLowConfidenceScreenProps {
  onNavigate: (screen: string) => void;
  onSavePhoto: (photoData: any) => void;
  capturedImage: string | null;
}

export function CaptureReviewLowConfidenceScreen({ onNavigate, onSavePhoto, capturedImage }: CaptureReviewLowConfidenceScreenProps) {
  const [autoCrop, setAutoCrop] = useState(false); // Default to OFF for low confidence
  ...
  // Hide helper text after 2 seconds
  useEffect(() => {
  ...
  return (
    <div className="w-full h-full min-h-screen bg-gradient-to-b from-[#0A0A0A] to-[#1A1A1A] relative overflow-hidden max-w-[393px] mx-auto">
      ...
      <div className="absolute top-0 left-0 right-0 z-30 bg-gradient-to-r from-orange-500 to-orange-600 text-white shadow-lg">
        <div className="flex items-center justify-between px-5 py-4 pt-16">
          <div className="flex items-center space-x-3">
            <AlertTriangle className="w-5 h-5 flex-shrink-0" />
            <span className="text-sm font-medium tracking-wide">COULDN'T DETECT EDGES ACCURATELY</span>
          </div>
          <Button
            variant="ghost"
            onClick={handleTryAgain}
            className="text-white hover:bg-white/20 px-4 py-2 h-9 text-sm font-medium rounded-lg transition-all duration-200"
          >
            Try again
          </Button>
        </div>
      ...
      {/* Top Bar - Overlay Style with adjusted positioning */}
      <div className="absolute top-16 left-0 right-0 z-20 bg-gradient-to-b from-black/60 via-black/30 to-transparent">
      ...
      {/* Image Preview Area */}
      <div className="relative w-full h-full flex items-center justify-center px-8 pt-16">
        ...
        >
          ...
          {/* Manual Crop Overlay - Poor detection */}
          <div className="absolute inset-0 pointer-events-auto">
            {/* Irregular/poor detection polygon */}
            <svg className="absolute inset-0 w-full h-full">
              ...
              <polygon
                points="60,80 220,65 250,310 30,350"
                fill="none"
                stroke="#FF6B6B"
                strokeWidth="2"
                strokeDasharray="8,6"
            ...
            {/* Corner Handles - Error state styling */}
            <div className="absolute w-4 h-4 bg-red-400 rounded-full shadow-lg border-2 border-red-600 cursor-grab hover:cursor-grabbing hover:scale-110 transition-all duration-200" style={{ top: '78px', left: '58px' }}></div>
            <div className="absolute w-4 h-4 bg-red-400 rounded-full shadow-lg border-2 border-red-600 cursor-grab hover:cursor-grabbing hover:scale-110 transition-all duration-200" style={{ top: '63px', left: '218px' }}></div>
            <div className="absolute w-4 h-4 bg-red-400 rounded-full shadow-lg border-2 border-red-600 cursor-grab hover:cursor-grabbing hover:scale-110 transition-all duration-200" style={{ bottom: '108px', left: '248px' }}></div>
            <div className="absolute w-4 h-4 bg-red-400 rounded-full shadow-lg border-2 border-red-600 cursor-grab hover:cursor-grabbing hover:scale-110 transition-all duration-200" style={{ bottom: '68px', left: '28px' }}></div>
          </div>

          {/* Rule of Thirds Grid - Higher contrast for precision */}
          {showGrid && (
            <div className="absolute inset-0 pointer-events-none">
              <svg className="w-full h-full">
                {/* Vertical lines */}
                <line x1="33%" y1="0%" x2="33%" y2="100%" stroke="white" strokeWidth="1" opacity="0.7" />
                <line x1="66%" y1="0%" x2="66%" y2="100%" stroke="white" strokeWidth="1" opacity="0.7" />
                {/* Horizontal lines */}
                <line x1="0%" y1="33%" x2="100%" y2="33%" stroke="white" strokeWidth="1" opacity="0.7" />
                <line x1="0%" y1="66%" x2="100%" y2="66%" stroke="white" strokeWidth="1" opacity="0.7" />
              </svg>
            </div>
          )}
        </div>

        {/* Drag Corners Helper Text */}
        {showHelperText && (
      ...
      <button
        ...
        className={`absolute top-32 right-5 w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-200 backdrop-blur-sm border ${
          showGrid 
